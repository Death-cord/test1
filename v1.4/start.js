...
function fetch_default_mpqs() {
	...
	mpq_retry_attempt += 1;
	mpq_status_lines = ["Loading MPQ files from ./bw/ folder", "Attempt " + mpq_retry_attempt + "."];
	...
	var on_complete = function() {
		...
		if (failed || !has_all_files()) {
			mpq_status_lines.push("❌ MPQ files NOT found in ./bw/");
			mpq_status_lines.push("Required files:");
			mpq_status_lines.push("  • ./bw/STARDAT.MPQ");
			mpq_status_lines.push("  • ./bw/BROODAT.MPQ");
			mpq_status_lines.push("  • ./bw/patch_rt.mpq");
			show_loading_files_screen(mpq_status_lines.join('<br>'));
			return;
		}
	...
	// ✅ Load from ./bw/ folder (NOT from network)
	for (var i = 0; i < C_MPQ_FILENAMES.length; ++i) {
		(function(index) {
			...
			req.onreadystatechange = function() {
				...
				if (req.status === 200) {
					...
					update_mpq_loading_status(C_MPQ_FILENAMES[index] + ": ✓");
				} else {
					failed = true;
					console.log("Failed to load from " + C_DEFAULT_MPQ_SOURCES[index]);
					update_mpq_loading_status(C_MPQ_FILENAMES[index] + ": ✗");
				}
				on_complete();
			};
			...
			req.timeout = 10000;  // 10 second timeout for local files
